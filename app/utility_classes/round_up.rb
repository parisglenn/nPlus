...
class BestPairings
	def initialize round_up, match_groups, random
		...
		@matched_users = [] #one dimensional array
...
class GenerateMatches
	...
	def commit_pairs_to_db best_pairing
		best_pairing.each do |pair, avail|
			rum = RoundUpMatch.new #:date, :expires, :round_up_time_id
			#RoundUpMatchUser.new :round_up_match_id, :rsvp, :user_id
			RoundUpMatchUser.new user_id: least_available_users[0], office_id: key.office_id, round_up_time_id: key.round_up_time_id
			RoundUpMatchUser.new user_id: user.id, office_id: key.office_id, round_up_time_id: key.round_up_time_id
		end
	end
...
create arrays containing users for all round up times by office and time
for each user - for each group - figure out how many matches they have
